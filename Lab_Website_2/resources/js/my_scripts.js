...
function loadStatsPage(){
	...
	for (var number = 2; number < table.rows.length; number++){
		var row = table.rows[number];
		var home = parseInt(row.cells[2].innerHTML);
		var opp = parseInt(row.cells[3].innerHTML);
		if (home > opp){
	...
	var lossing = document.getElementById("losses");
	lossing.innerHTML += loss;
}
...
function loadPlayersPage(){
  ...
  for(var i = 0; i < players.length; i++){
		var playing = players[i].name;
    count = count + "<a class = 'dropdown-item' href='#' onclick = 'switchPlayers("+ i +")'>" + playing +"</a>"
	}
	playerSelect.innerHTML = count;
}
...
function switchPlayers(playerNum){
  ...
  var receciningYards = Math.floor(players[playerNum].receiving_yards/players[playerNum].games_played);
  document.getElementById("avg_p_yards").innerHTML= passingYards;
  document.getElementById("avg_r_yards").innerHTML= passingYards;
	document.getElementById("avg_rec_yards").innerHTML= passingYards;
}
